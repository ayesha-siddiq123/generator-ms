...
def add_processor_group(pg_name):
    ...
    res = rq.post(f'http://{nifi_host}:{nifi_port}/nifi-api/process-groups/{nifi_root_pg_id}/process-groups', json=pg_details)
    if res.ok:
...
def add_processor(processor_group_name, processor_name, name):
    ...
    processor_res = rq.post(
        f"http://{nifi_host}:{nifi_port}/nifi-api/process-groups/{pg_source_json['processGroupFlow']['id']}/processors",
        json=processors)
...
def connect(processor_group_name,sourceid, destinationid, relationship):
    ...
    connection = rq.post(
        f"http://{nifi_host}:{nifi_port}/nifi-api/process-groups/{pg_source_json['processGroupFlow']['id']}/connections",
        json=json_body)
    if connection.ok:
...
def add_ports(processor_group_name, name):
    ...
    if name.__contains__('output'):
        processor_res = rq.post(f"http://{nifi_host}:{nifi_port}/nifi-api/process-groups/{pg_id}/output-ports",
            json=processors)
        if processor_res.status_code == 201:
    ...
    if name.__contains__('input'):
        processor_res = rq.post(f"http://{nifi_host}:{nifi_port}/nifi-api/process-groups/{pg_id}/input-ports",
            json=processors)
        if processor_res.status_code == 201:
...
def connect_ports_inside_pg(processor_group_name,sourceid, source_groupid, source_type, destinationid, destination_groupid,destination_type,relationship):
    ...
    if destination_type == 'FUNNEL':
        connection = rq.post(
            f"http://{nifi_host}:{nifi_port}/nifi-api/process-groups/{nifi_root_id}/connections",
            json=json_body)
        if connection.ok:
        ...
        connection = rq.post(
            f"http://{nifi_host}:{nifi_port}/nifi-api/process-groups/{processor_group_id}/connections",
            json=json_body)
        if connection.ok:
            print(f"Successfully connected from {sourceid} to {destinationid}")
        else:
...
def create_funnel():
    ...
    connection = rq.post(
        f"http://{nifi_host}:{nifi_port}/nifi-api/process-groups/{get_root_id}/funnels",
        json=payload)
    if connection.ok:
