...
def linked_qr_count():
    ...
    df_snap.columns = ['textbook_id','grade_diksha','subject_diksha','medium','linked_qr_count']
    obj.upload_file(df_snap,'linkedqrcount-event.data.csv')

def resource_count():
    df_snap = df_data[['TB Id','Grade','Subject','Medium','Resource Count']]
    df_snap.columns = ['textbook_id','grade_diksha','subject_diksha','medium','resource_count']
